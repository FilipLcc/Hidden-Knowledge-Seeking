...
def data_regulation(marke_nazivi, marke_vrednosti, initial_dataset):
...
#data fetching
mycursor.execute('select marka, kubikaza, kilometraza, godiste, snaga, cena from big_kola')
initial_dataset = mycursor.fetchall()
#data regulation
data_regulation(marke_nazivi, marke_vrednosti, initial_dataset)
#data scaling
max_values = [0,0,0,0,0,0]#to remember what to multiply the later results with
...
#initialize vectors
w = [1, 0.2, 0.4, -0.8, 0.7, 0.1]#w0, w1, w2, w3, w4, w5, w6
temp_w = [0, 0, 0, 0, 0, 0]
#0.1 or 0.01
a = 0.1
# # w = w0,w1...wn
...
def train_a_set(set):
    i = 0
    prim_values = [1, 1 ,1 ,1,1,1]
    while (abs(prim_values[0]) > 0.00000025 and abs(prim_values[1]) > 0.00000025  
     and abs(prim_values[2]) > 0.00000025  and abs(prim_values[3]) > 0.00000025 
     and abs(prim_values[4]) > 0.00000025  and abs(prim_values[5]) > 0.00000025): # or i > 1000000000
...
for i in range(len(test_set)):
    predictions.append(h(test_set[i])*max_values[5])
    true_values.append(test_set[i][5]*max_values[5])
train_set_true_values = []
for i in range(len(train_set)):
    train_set_true_values.append(train_set[i][5]*max_values[5])
